...
module.exports = {
   ...
   edges: {
      ...
      "_e_init_settle": {
         ...
         evaluate: function (f) { return f.isFirstTurn || f.isSecondTurn; },
         arguments: "int",
         execute: function (g,a) { let p=_.validatePlayer(g,a[0]), j=_.validateJunc(g,a[1]); if (!j.isSettleable) throw Error('junc cannot be settled'); g.state.players[p].settlements.push(j.num); j.owner = p; j.isSettleable = false; _.juncGetAdjJuncs(g.board,j.num).forEach( function(adj) { g.board.juncs[adj].isSettleable = false; }); },
         isPriority: false,
         isMulti: false,
         isCancel: false,
         label: ""
