...
module.exports = {
   ...
   edges: {
      ...
      "_e_init_build_road": {
         ...
         "arguments": "int",
         execute: function (g,a) { let p=a[0], r=funcs.validateRoad(g,a[1]), valid=false, match=g.state.players[p].settlements.slice(-1).pop(); if (r.owner > -1) throw Error('road is already owned'); for (let j in r.juncs) { if (r.juncs[j] === match) valid=true; } if (!valid) throw Error('this junc is not adjacent'); },
         isPriority: false,
         isMulti: false,
         isCancel: false,
         label: ""
      ...
      "_e_init_settle": {
         ...
         evaluate: function (f) { return f.isFirstTurn || f.isSecondTurn; },
         "arguments": "int",
         execute: function (g,a) { let p=a[0], j=funcs.validateJunc(g,a[1]); if (!j.isSettleable) throw Error('junc cannot be settled'); g.state.players[p].settlements.push(j.num); j.owner = p; j.isSettleable = false; funcs.juncGetAdjJuncs(g.board,j.num).forEach( function(adj) { g.board.juncs[adj].isSettleable = false; }); },
         isPriority: false,
         isMulti: false,
         isCancel: false,
         label: ""
